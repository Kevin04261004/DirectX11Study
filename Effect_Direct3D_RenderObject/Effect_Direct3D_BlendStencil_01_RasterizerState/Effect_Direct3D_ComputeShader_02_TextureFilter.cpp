...
{
	mMainWndCaption = L"CK_DX_RenderObject Demo";
	mUseCS = false;
	mEnable4xMsaa = false;
	mKeyPressed = false;
	mLastMousePos.x = 0;
...
void CK_DX_ComputeTexFilter::UpdateScene(float dt)
{
	...
	else if (mBoxPosition >= 1)
	{
		mDir = -1;
	}
	mBoxPosition += mDir * moveSpeed * dt;
...
void CK_DX_ComputeTexFilter::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		scalM = XMMatrixScaling(2.0f, 1.0f, 1.0f);
		transM = XMMatrixTranslation(mBoxPosition, 0, 0);
		finalM = scalM * transM;
...
void CK_DX_ComputeTexFilter::BuildInBuffer()
{
	...
	HR(CreateDDSTextureFromFile(md3dDevice, L"Textures/brick01.dds", (ID3D11Resource**)(&mWheelSrcTexture), &mWheelTextureSRV));
